"""
nodes.py -- the maze as a graph of nodes that entities travel between.
...
A NodeGroup parses a maze text file: every node symbol ('+', 'P', 'n') becomes a
Node, and runs of node/path symbols on the same row/column are linked as LEFT-RIGHT
and UP-DOWN neighbours. Nodes are keyed by pixel coordinates. Per-direction
`access` lists control which entities may take each edge (e.g. ghosts are barred
from re-entering the ghost house through certain edges).
"""
import pygame
...
class NodeGroup(object):
    ...
    def connectHorizontally(self, data, xoffset=0, yoffset=0):
        # Walk each row left to right, linking consecutive nodes as LEFT/RIGHT
        # neighbours. Path symbols continue a run; anything else breaks it (wall).
        for row in list(range(data.shape[0])):
            key = None
            for col in list(range(data.shape[1])):
                if data[row][col] in self.nodeSymbols:
    ...
    def connectVertically(self, data, xoffset=0, yoffset=0):
        # Same idea as connectHorizontally but down each column; transposing the
        # grid lets us reuse the row-walking logic.
    ...
    def setPortalPair(self, pair1, pair2):
        # Link two tunnel-mouth nodes as each other's PORTAL neighbour so an
        # entity stepping onto one is teleported to the other (screen wrap-around).
