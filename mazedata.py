"""
mazedata.py -- per-maze layout metadata (portals, ghost house, start positions).
...
The maze *graph* comes from the text files (see nodes.py); this module holds the
extra hand-tuned data each maze needs: portal-pair tile coordinates, the ghost
house offset and connections, Pac-Man/fruit start tiles, and which edges ghosts
are denied. MazeData.loadMaze(level) picks Maze1 or Maze2, cycling by level.
"""
